...
//! one:
//!
//! - This shares the formatter's front end, so it can only catch the
//!   *emitter*. A lexer that misreads a character moves both sides of the
//!   comparison identically and reads as agreement: when a lone `\r` was
//!   briefly treated as a line ending, `# c\rmessage(x)` uncommented the
//!   `message` and this saw nothing, because it ended the comment in the same
//!   wrong place on the output. Closing that means a second reader, which is
//!   not what this module is.
//! - A comment's position within a command is not compared, because the
//!   formatter re-places one legitimately.
//! - Neither the reordering canonicalisation nor the closer exemption knows
//!   about suppression, so inside a `# cmake-fmt: off` region — or after
//!   `no-sort` or `skip` — this would accept a permutation or a rewritten
//!   closer that the formatter must never produce there. Closing that means
//!   giving this module the same region tracking the formatter has.
//! - A bracket comment's trailing whitespace is normalised away on both sides,
//!   so a formatter that *added* some inside one would pass. The exemption is
//!   there because the formatter removes it, and this half of it is untested by
//!   construction.
//! - `normalize_line_endings` forgives `\r\n` against `\n` on both sides, and
//!   `line_ending = crlf` rewrites every `\n` in the finished buffer. A lone
//!   `\n` *inside* a quoted or bracket argument is therefore rewritten to
//!   `\r\n` — a byte added to a value — and this cannot see it. Closing that
//!   means emitting the configured line ending where the renderer produces one,
//!   rather than replacing them all afterwards.
...
fn significant(
    ...
    match token.kind() {
        ...
        // A bracket comment keeps its hashes — `#[[` is fixed — but loses
        // trailing whitespace on every line it spans, exactly as a line comment
        // does. Comparing it verbatim meant the one cosmetic rule had two
        // answers here, and the formatter's own trim read as content loss.
        SyntaxKind::BRACKET_COMMENT => Some(
            token
                .text()
                .split('\n')
                .map(str::trim_end)
