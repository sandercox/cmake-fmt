...
fn run_with_stdin_in(dir: Option<&std::path::Path>, args: &[&str], input: &str) -> (bool, String) {
    ...
    // A run that rejects its arguments exits before reading stdin, so this write
    // can legitimately hit EPIPE -- every `--ignore-file` refusal below is such
    // a run. Only that error is tolerated; what each test asserts on is the
    // child's own exit code and output.
    match child
        .stdin
        .as_mut()
        .expect("Failed to get stdin")
...
fn test_assume_filename_agrees_with_directory_walk() {
    ...
    for (walk_root, scope) in scopes {
        ...
        // The report names paths with the platform's separator, and `scope`
        // spells them with `/`, so on Windows a plain `contains` said the walk
        // had not seen a file it had just listed.
        let walk_report_slashed = walk_report.replace('\\', "/");
        for file in scope {
            let walk_sees = walk_report_slashed.contains(file);
...
fn test_an_unreadable_ignore_file_fails_the_run() {
    ...
    // The reason is asserted, not just the refusal. Deleting the `is_dir` arm
    // still refuses a directory — `File::open` succeeds on one and `io::copy`
    // then fails EISDIR — so the exit code alone cannot see that arm go.
    for (arg, reason) in [("typo.txt", "cannot be read"), (".", "is a directory")] {
        let (code, stderr) = refused(&["-r", "--check", ".", "--ignore-file", arg]);
        assert_eq!(code, Some(1), "{:?} should fail the run: {}", arg, stderr);
        assert!(
            stderr.contains("--ignore-file") && stderr.contains(reason),
...
/// Whether `script` accepts these arguments.
#[cfg(unix)]
fn script_accepts(args: &[&str]) -> bool {
    Command::new("script")
        .args(args)
...
fn test_a_terminal_ignore_file_is_read_once() {
    ...
    // Only util-linux's `-qec CMD FILE` is usable. BSD `script`, which macOS
    // ships, takes the command after the file — but it also echoes what it
    // reads back through the pty, so the pattern arrives as `^Dbuild/`, matches
    // nothing, and the test would be asserting the echo rather than the
    // regression. Probed rather than guessed from the platform, since either
    // can be installed.
    if !script_accepts(&["-qec", "true", "/dev/null"]) {
        return;
    }
    ...
    command.arg("-c").arg(shell).current_dir(root);
...
#[test]
#[cfg(unix)]
fn test_a_walk_warning_keeps_the_spelling_you_gave() {
    // Roots are resolved before the walk so a verdict belongs to the directory
    // rather than to how it was spelled. That left the walk's own complaints
    // naming an absolute path nobody typed, while `Would reformat:` kept the
    // user's — and the CHANGELOG promises the spelling for everything reported.
    use std::os::unix::fs::PermissionsExt;
    let tempdir = TempDir::new().expect("tempdir");
    let root = tempdir.path();
    std::fs::write(root.join("top.cmake"), "set(A   b)\n").expect("write");
    std::fs::create_dir(root.join("y")).expect("mkdir");
    std::fs::set_permissions(root.join("y"), std::fs::Permissions::from_mode(0o000))
        .expect("chmod");
    // Root ignores the mode, so only assert when the walk really fails
    if std::fs::read_dir(root.join("y")).is_ok() {
        return;
    }
    ...
    assert!(
        stderr.contains("./y"),
        "the warning lost the spelling that was given:\n{}",
        stderr
    );
    assert!(
        !stderr.contains(root.to_str().expect("utf-8 tempdir")),
        "the warning names the resolved absolute path:\n{}",
        stderr
    );

    std::fs::set_permissions(root.join("y"), std::fs::Permissions::from_mode(0o755)).ok();
}

/// A working directory whose spelling differs from its resolved form.
///
/// On Unix `getcwd()` is physical, so a symlinked directory is the only way in;
/// on Windows a junction, an 8.3 component, a `subst` drive or a mapped network
/// drive all do it, and GitHub's runner has an 8.3 `%TEMP%`. No developer
/// machine reproduces it by accident, which is why every spelling bug in this
/// module went unnoticed until CI ran on Windows.
///
/// Returns `(spelled, resolved)` directories naming the same place, or `None`
/// where the platform gives no way to make one.
fn a_directory_with_two_spellings(
    root: &std::path::Path,
    ...
    link.exists().then_some((link, real))
}

#[test]
fn test_an_anchored_ignore_file_pattern_survives_an_unresolved_cwd() {
    // `WalkBuilder::add_ignore` anchors its matcher at the walker's working
    // directory and `Gitignore::strip` removes that prefix before matching, so
    // an *anchored* pattern only fires when that spelling agrees with the roots
    // the walk was given. Those roots are resolved, so the walker has to be
    // told the resolved working directory too — otherwise nothing is stripped,
    // `gen/*` is matched against a full absolute path where it can never fire,
    // and every file it excludes is formatted. With `-i`, rewritten.
    //
    // The pattern has to be anchored: an unanchored `gen/` matches a basename
    // at any depth and works either way, which is why the rest of the suite
    // could not see this.
    let tempdir = TempDir::new().expect("tempdir");
    let Some((spelled, resolved)) = a_directory_with_two_spellings(tempdir.path()) else {
        return; // no way to make two spellings here
    };

    std::fs::create_dir_all(resolved.join("gen")).expect("mkdir");
    std::fs::write(resolved.join("ig.txt"), "gen/*\n").expect("write");
    for file in ["kept.cmake", "gen/excluded.cmake"] {
        std::fs::write(resolved.join(file), "set(A   b)\n").expect("write");
    }

    // Once through each spelling of the very same directory. The verdict
    // belongs to the directory, so both must reach it.
    for cwd in [&resolved, &spelled] {
        let output = Command::new(cmake_fmt_bin())
            .args(["--check", "-r", ".", "--ignore-file", "ig.txt"])
            .current_dir(cwd)
            .output()
        ...
        assert!(
            report.contains("kept.cmake"),
            "the walk reached nothing from {}:\n{}",
            cwd.display(),
            report
        );
        assert!(
            !report.contains("gen/excluded.cmake"),
            "an anchored --ignore-file pattern stopped applying from {}:\n{}",
            cwd.display(),
            report
        );
    }
...
#[test]
fn test_the_ignore_file_cwd_bound_survives_an_unresolved_cwd() {
    // `--ignore-file` is bounded by the working directory, and the target is
    // resolved — so the bound has to be resolved too. `current_dir()` is
    // physical on Unix but hands back the caller's spelling on Windows, so an
    // unresolved bound never contained a resolved target there and no
    // `--ignore-file` rule was consulted at all: the file was formatted rather
    // than passed through.
    //
    // The walker-anchoring test above does not cover this — that one is about
    // what the `ignore` crate derives for itself, this is about our own bound —
    // and with this fix reverted the whole suite stayed green.
    let tempdir = TempDir::new().expect("tempdir");
    let Some((spelled, resolved)) = a_directory_with_two_spellings(tempdir.path()) else {
        return; // no way to make two spellings here
    };

    std::fs::create_dir_all(resolved.join("sub")).expect("mkdir");
    std::fs::write(resolved.join("ig.txt"), "sub/\n").expect("write");
    let unformatted = "set(FOO   bar)\n";
    std::fs::write(resolved.join("sub").join("x.cmake"), unformatted).expect("write");

    // The editor hands over the path the user opened, so the spelled one.
    let target = spelled.join("sub").join("x.cmake");
    let (ok, stdout) = run_with_stdin_in(
        Some(&spelled),
        &[
            "-",
    ...
    assert!(ok, "the run failed outright");
    assert_eq!(
        stdout, unformatted,
        "an excluded file was formatted, so the --ignore-file bound never contained it"
    );
}

#[test]
// `/dev/null` and inode identity are both Unix notions.
#[cfg(unix)]
fn test_sharing_an_inode_with_stdin_is_not_enough_to_be_refused() {
    // The negative control the check above cannot give: it puts stdin on a pipe,
    // so a `(dev, ino)` identity test passes it whether or not the file type is
    // considered. Both of these name the very same inode as stdin and neither
    // loses a byte, because a regular file is reopened at its own offset and
    // `/dev/null` yields EOF without consuming anything.
    //
    // `< /dev/null` is what a cron job, a systemd unit and a Makefile recipe
    // give every process, and the README promises `/dev/null` works.
    let tempdir = TempDir::new().expect("tempdir");
    let root = tempdir.path();
    std::fs::write(root.join("ig.txt"), "build/\n").expect("write");

    // (the --ignore-file argument, the path to hand to stdin)
    let cases = [
        ("/dev/null", std::path::PathBuf::from("/dev/null")),
        ("ig.txt", root.join("ig.txt")),
    ];

    for (ignore_file, stdin_path) in cases {
        let stdin = std::fs::File::open(&stdin_path).expect("open the file stdin will be");
        let output = Command::new(cmake_fmt_bin())
            .args([
                "-",
                "--assume-filename",
...
#[test]
// A fifo and the deadline helper are both Unix-only; the respelling itself is
// covered on every platform by the tests above.
#[cfg(unix)]
