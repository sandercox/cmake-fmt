...
fn test_generate_diff_with_changes() {
    ...
    // Verify headers
    assert!(
        diff_text.contains("--- a/test.cmake"),
        "Missing '---' header"
    );
    assert!(
        diff_text.contains("+++ b/test.cmake"),
        "Missing '+++' header"
    );
    ...
    // Verify removed line
    assert!(
        diff_text.contains("-set(FOO   bar)"),
...
fn test_generate_diff_header_format() {
    ...
    assert!(
        diff_text.contains("--- a/dir/subdir/file.cmake"),
        "Path should use forward slashes, got:\n{}",
        diff_text
    );
    assert!(
        diff_text.contains("+++ b/dir/subdir/file.cmake"),
        "Path should use forward slashes, got:\n{}",
        diff_text
    );
    ...
    assert!(
        !first_line.contains('\\'),
        "Header should not contain backslashes"
    );
}
...
fn test_generate_diff_stdin_path() {
    ...
    assert!(
        diff_text.contains("--- a/stdin"),
        "Missing stdin in --- header"
    );
    assert!(
        diff_text.contains("+++ b/stdin"),
        "Missing stdin in +++ header"
    );
}
...
fn test_generate_diff_patch_compatible() {
    ...
    assert!(
        diff_text.contains("# Line 1")
            || diff_text.contains("# Line 2")
            || diff_text.contains("# Line 3"),
        "Should include context lines before change"
    );
    assert!(
        diff_text.contains("# Line 5")
            || diff_text.contains("# Line 6")
            || diff_text.contains("# Line 7"),
...
fn test_diff_stdin() {
    ...
    assert!(
        ...
        "Diff headers should reference 'stdin'"
    );
...
fn test_diff_multiple_files() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("file1.cmake"),
        "Should show diff for file1.cmake"
    );
    assert!(
        stdout.contains("file2.cmake"),
        "Should show diff for file2.cmake"
    );
...
fn test_diff_multiline_changes() {
    ...
    assert!(
        ...
        "Should show old FOO line"
    );
    assert!(stdout.contains("+set(FOO bar)"), "Should show new FOO line");
    assert!(
        stdout.contains("-set(BAZ   qux)"),
        "Should show old BAZ line"
    );
