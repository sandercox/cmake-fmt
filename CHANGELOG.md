...
## [Unreleased]
### Fixed
- A comment inside a keyword section is no longer deleted, and a property's value never lands inside one. The shortcuts that put a single value inline with its keyword emitted nothing else, so `target_sources(t PRIVATE\n\t# impl\n\tb.cpp)`, `list(APPEND V\n\t# note\n\ta.cpp)`, a one-pair `PROPERTIES` run and a valueless flag such as `find_package(Foo REQUIRED # note ...)` all lost their comments. A `PROPERTIES` key's trailing comment is now emitted after the value rather than before it, where it commented the value out
