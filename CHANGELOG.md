...
## [Unreleased]
### Fixed
- A comment written on its own line inside a keyword section is no longer deleted. The shortcut that puts a single value inline with its keyword emitted nothing else, so `target_sources(t PRIVATE\n\t# impl\n\tb.cpp)` lost `# impl` outright
