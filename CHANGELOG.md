...
- `.cmake-fmt-ignore` (and `--ignore-file`) now apply to stdin input with `--assume-filename`, so editor format-on-save skips ignored files instead of formatting them. Matching follows git's rule that an excluded directory is final, so `build/` is not undone by a later `!build/keep.cmake` ([#4](https://github.com/sandercox/cmake-fmt/issues/4))
- A directory excluded by an ignore file is skipped however you reach it — named on the command line, walked from above, walked from inside, reached through a symlink, or piped through stdin
- `--ignore-file` pointing at something that is not a readable file is now an error instead of a warning. A typo there formatted every file you meant to exclude and exited 0
