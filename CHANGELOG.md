...
- A directory named as the walk root is resolved before it is walked, so `cmake-fmt -r sub`, `-r sub/../sub` and `-r link-to-sub` reach the same verdict as each other and as the stdin path. An `--ignore-file` pattern anchored to its root previously excluded one spelling and not another
...
- A file that cannot be read or written now fails the run instead of printing a message and exiting 0
