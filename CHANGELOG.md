...
- A carriage return inside a value is no longer deleted. Every `\r` was stripped before parsing, which reached into a quoted and a bracket argument exactly as the whitespace strip did, so `set(A "a\rb")` came back as `set(A "ab")` — and the content guard could not see it, because it stripped both sides the same way. Only `\r\n` is a line ending now; a lone `\r` is *space*, which is what CMake calls it — `cmake -P` rejects `set(A 1)\rmessage(x)` as a parse error, so the `\r` does not end the command, and it does not end a comment either
- Two identical comments in one file are both written. The set of comments already emitted with their command was keyed on the comment's *text*, so a second `# note` anywhere in the file read as already handled and was dropped. The content guard added in this release catches it and refuses the file, so the visible symptom is a valid file that cannot be formatted at all
