...
### Added
- The formatter now checks its own output before writing: if formatting would change what a file *says* rather than how it looks, the file is left exactly as written, a warning names what changed, and the run exits non-zero. Re-indenting, re-wrapping, re-casing, `closing_style` and the reordering passes are all recognised as permitted; inventing or dropping an argument is not
